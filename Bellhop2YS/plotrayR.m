function varargout = plotrayR( rayfil, dstep )
...
for isz = 1 : Nsz
    for ibeam = 1 : Nalpha
        ...
        %ii = mod( ii, 3 ) + 1;
%         plot( r, z, lincol( ii ) );
%         if(z(end)>=dstep-15&&z(end)<=dstep+15) %会聚区z(end)>=100&&z(end)<=150
            if NumTopBnc >= 1 && NumBotBnc >= 1
                plot( r, z, '-k', 'LineWidth', 0.5 )    % hits both boundaries
            elseif NumBotBnc >= 1
                plot( r, z, '-b', 'LineWidth', 1 )	 % hits bottom only
            elseif NumTopBnc >= 1
                plot( r, z, '-g',  'LineWidth', 1 )	 % hits surface only
            else
                plot( r, z, '-r',  'LineWidth', 1.2 )    % direct path
            end
            valid_src_angles = [valid_src_angles, alpha0];  % 记录发射角
%         end
        % update axis limits
        rmin = min( [ r rmin ] );
        rmax = max( [ r rmax ] );
...
fclose( fid );
save('valid_angles.mat', 'valid_src_angles');
hold off
zoom on
