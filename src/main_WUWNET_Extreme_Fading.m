...
% ==================== 参数设定 ====================
MONTE_CARLO_ITERS = 300; % 极值沙盒环境：减少迭代次数以快速验证深衰落下的 4.5 dB 性能鸿沟
...
plot_save_dir_base = '../results_plots/WUWNET_Extreme'; % 独立沙盒结果目录，不污染原版数据
...
for ch_idx = 1 : num_channels
    ...
    for s_idx = 1 : length(SNR_range)
        ...
        for trial = 1 : Num_Trials
            % ========================================================
            % [核心修改] 注入非平稳多普勒与瞬态深衰落 (逼迫基准算法发散)
            % ========================================================
            % 1. 随机游走多普勒相位漂移 (模拟海面波浪引起的时变多普勒，峰值 \pm 10 Hz)
            ...
            % 2. 构造包内突发深衰落 (模拟信号瞬时被遮挡或被多径相消，衰落达 -20dB)
            fade_envelope = ones(1, length(signal_channel));
            fade_center = round(length(signal_channel) * (0.3 + 0.4*rand())); % 发生在数据包 30%~70% 的位置
            fade_width = round(fs * 0.05 / 2); % 持续时间 50 ms
